...
<body>
	...
	<table class="table table-striped" style="width: 600px;">
		...
		<c:if test="${totalCount>0 }">
			<c:forEach var="dto" items="${list }">
				<tr>
					<!-- dto.num을 넣으면 되지만 만약 계시글이 삭제되면 num에 
						맞춰서 저장이 되기때문에 no에 번호에 넣어서 출력 
					-->
					<td>${no }</td>
					<!-- 증감연산자가 없으므로 1씩 감소후 저장 -->
	...
	이전을 클릭할경우 전블럭의 마지막 페이지가 현재페이지가 되어야한다
	 -->
		...
		<!-- 다음 : endpage가 totalpage보다 작으면 다음이 필요하다
			그런데 다음을 누를 경우 다음 블럭의 첫페이지가 현재페이지가 되어야만 한다		
		 -->
