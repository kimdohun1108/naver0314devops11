...
public class Ex1SwingDBStudent extends JFrame{
	...
	public Ex1SwingDBStudent(String title) {
	...
	public void initDesign()
	{
		...
		btnAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				try {
					...
					pstmt.setInt(2, Integer.parseInt(tfHtml.getText()));
					pstmt.setInt(2, Integer.parseInt(tfSpring.getText()));
		...
		btnDel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				if(row==-1)
				{
					JOptionPane.showMessageDialog(Ex1SwingDBStudent.this, "삭제할 행을 선택해주세요");
					return;
				}
				//row행의 0번열에 있는 시퀀스값을 얻는다 getValuesAt
				String num = table.getValueAt(row, 0).toString();
				
				//그 시퀀스에 해당하는 db데이터를 삭제하는 sql문 작성
				String sql = "dlete from student where num=?";
				
				//Connection, PreparedStatrment 생성후 sql문 실행
				Connection conn = db.getConnection();
				PreparedStatement pstmt = null;
	...
	public void writeStudent()
	{
		...
		try {
			...
			while(rs.next())
			{
				...
				nf.setMaximumFractionDigits(2);//소수점2자리
	...
	public static void main(String[] args) {
		...
		Ex1SwingDBStudent a=new Ex1SwingDBStudent("학생관리 DB");
	}
