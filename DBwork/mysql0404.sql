...
select date_format(now(),'%Y-%m-%d') from dual; -- 24-04-04
select date_format(now(),'%Y-%m-%d %H:%i') from dual; -- 24-04-04 10:08
select str_to_date(20240404,'%Y%m%d') from dual; -- 숫자를 날짜 타입으로 변환
...
-- test01 테이블 생성
 ...
 -- insert : 일부만 추가할 경우 컬럼명기재, 전체다줄경우 컬럼명 생략
 ...
 -- 컬럼 생략시 num도 값을 넣어야하는데  num은 auto이므로 그냥 null로 넣으면됨
 ...
 select * from test01 where name like '_i%'; -- 두번째 글자가 i인사람
 
 select upper(name),height from teat01; -- 이름은 대문자로
 select ucase(name),height from teat01; -- 이름은 대문자로
 ...
 -- 컬럼명 변경(mysql5는 change,mysql8은 rename column (오라클하고같다) 
 -- hp -> phone으로 변경 
 ...
 select * from test01 limit 3,2; -- 첫데이터가 0번:3번부터 2개 조회 
...
select * from sawon;
-- check에 안맞는값을 insert 할경우 오류메세지 확인
insert into sawon values (null,'이민정',110,'여자','홍보부'); -- 에러발생 (score 수 초과 )
insert into sawon values (null,'홍길동',87,'남','홍보부'); -- 에러발생(gender 명 오류)
-- check에 맞게 데이터 추가 
insert into sawon values (null,'이민정',90,'여자','홍보부'); 
insert into sawon values (null,'홍길동',87,'남자','홍보부'); 

insert into sawon values (null,'김도일',1,'여자','인사부'); 
insert into sawon values (null,'김도이',2,'여자','교육부'); 
insert into sawon values (null,'김도삼',3,'여자','교육부'); 
insert into sawon values (null,'김도사',4,'여자','인사부'); 
insert into sawon values (null,'김도오',5,'여자','교육부'); 
insert into sawon values (null,'김도육',6,'남자','인사부'); 
insert into sawon values (null,'김도칠',7,'남자','홍보부'); 
insert into sawon values (null,'김도팔',8,'남자','홍보부'); 

-- group 함수 
select count(*) from sawon; -- 총인원수 
select sum(score),avg(score) from sawon; -- 총점,평균 
select sum(score),round(avg(score),1) from sawon; -- 총점, 평균 

-- 성별 : 인원수 최고점수 최저점수 평균점수를 구하자 
select gender,count(*) 인원수,max(score) 최고점수,min(score) 최저점수,round(avg(score),1) 평균점수
from sawon group by gender;

-- 부서별 인원수와 최고점수 
select buseo 부서,count(*) 인원수,max(score) 최고점수 from sawon group by buseo;
...
select count(*) inwon,max(score) maxscore,
ceiling(avg(score)) avgscore
from sawon
group by buseo;

-- shop과 cart join
select cartnum, username, sangpum, price, color, date_format(guipday,'%Y-%m-%d %H:%i')
from shop, cart
...
select * from sawon where name like concat('%','김','%');